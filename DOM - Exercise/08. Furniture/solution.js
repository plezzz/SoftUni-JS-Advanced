function solve() {
    ...
    const demarkInputElements = () => { document.querySelector('input').disabled = false };

    const getInfoForNewFurniture = () => { return JSON.parse(document.querySelector('textarea').value) };
    ...
    const formingNewRow = (obj) => {
        ...
        htmlElem = {
            'img': function (obj, key) { return `<td><img src="${obj[key]}"></td>` },
            'consistStartAndEndTag': function (obj, key) { return `<td><p>${obj[key]}</p></td>` }
        };
    ...
    const getMarkedFurniture = () => { return [...document.getElementsByTagName('input')].filter(el => el.checked === true) }

    const generateTable = () => {
        demarkInputElements();
        getInfoForNewFurniture().map(el => { addNewRowInTable(formingNewRow(el)) })
    };
    ...
    const addInfoInTextArea = (infoForBoughtFurnitures) => {
        ...
        let output = ['Bought furniture: ', 'Total price: ', 'Average decoration factor: ']
            .map((el, ind) => { return el += String(infoForBoughtFurnitures[ind]) }).join('\n');
